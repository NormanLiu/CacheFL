...
def one_inner_outer_iteration2(x_start_latest, x_start_delayed, Kcache, Kserver, K, stepsize, r):
...
def one_inner_outer_iteration3(x_start, M, K, stepsize, r):
    grads = [np.zeros_like(x_start[i]) for i in range(len(x_start))]
    for m in range(M):
        if m in sampledClientList[r]:
            x = x_start.copy()
...
def inner_outer_sgd(x0_len, Kset, K, R, inner_stepsize, outer_stepsize, loss_freq, avg_window=8):
    ...
    for r in range(R):
        ...
        direction = one_inner_outer_iteration(iterates[-1], Kset, K, inner_stepsize, r)
...
def inner_outer_sgd2(x0_len, Kcache, Kserver, K, R, inner_stepsize, outer_stepsize, loss_freq, avg_window=8):
    ...
    for r in range(R):
        if len(iterates) >= avg_window:
            iterates = iterates[-(avg_window-1):]
        if r>1:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-2], Kcache, Kserver, K, inner_stepsize, r)
        else:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-1], Kcache, Kserver, K, inner_stepsize, r)
        current_iterate = iterates[-1]
        iterates.append([(current_iterate[i] - outer_stepsize * direction[i]) for i in range(len(current_iterate))])
        if (r+1) % loss_freq == 0:
            loss, acc = evaluate(iterates[-1], features, labels_array)
...
def inner_outer_sgd3(x0_len, M, K, R, inner_stepsize, outer_stepsize, loss_freq, avg_window=8):
    ...
    for r in range(R):
        if len(iterates) >= avg_window:
            iterates = iterates[-(avg_window-1):]
        direction = one_inner_outer_iteration3(iterates[-1], M, K, inner_stepsize, r)
        current_iterate = iterates[-1]
        iterates.append([(current_iterate[i] - outer_stepsize * direction[i]) for i in range(len(current_iterate))])
        if (r+1) % loss_freq == 0:
            loss, acc = evaluate(iterates[-1], features, labels_array)
...
def inner_outer_sgd4(x0_len, M, K, R, inner_stepsize, outer_stepsize, loss_freq, avg_window=8):
    ...
    for r in range(R):
        if len(iterates) >= avg_window:
            iterates = iterates[-(avg_window-1):]
        if r>1:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-2], Kcache_list[r//10], Kserver_list[r//10], K, inner_stepsize, r)
        else:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-1], Kcache_list[r//10], Kserver_list[r//10], K, inner_stepsize, r)
        current_iterate = iterates[-1]
        iterates.append([(current_iterate[i] - outer_stepsize * direction[i]) for i in range(len(current_iterate))])
        if (r+1) % loss_freq == 0:
            loss, acc = evaluate(iterates[-1], features, labels_array)
...
learning_rate = sys.argv[2] if len(sys.argv) > 2 else 0.04
...
# sampledClientList = []
# for i in range(200):
#     sampledClient = np.random.choice(range(50), 30, replace=False)
#     sampledClientList.append(sorted(sampledClient))
#
# np.savez('sampledClient', sampledClientList = sampledClientList)
npzfile = np.load('sampledClient.npz')
sampledClientList = npzfile['sampledClientList']
...
n_sample_fraction = [1/M for i in range(M)]
...
experiment(M,K=5,R=200,plotfile= plot_prefix + '/smallM-smallK-' + distribution + dataset_name + alg_sel + str(learning_rate) + 'sampling-CCN.png',savearray=plot_prefix + '/smallM-smallK-' + distribution + dataset_name + alg_sel + str(learning_rate) + 'sampling-CCN')
