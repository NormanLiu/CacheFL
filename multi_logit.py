...
def inner_outer_sgd4(x0_len, M, K, R, inner_stepsize, outer_stepsize, loss_freq, avg_window=8):
    ...
    for r in range(R):
        if len(iterates) >= avg_window:
            iterates = iterates[-(avg_window-1):]
        if r>1:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-2], Kcache_list[r//10%20], Kserver_list[r//10%20], K, inner_stepsize)
        else:
        	direction = one_inner_outer_iteration2(iterates[-1], iterates[-1], Kcache_list[r//10%20], Kserver_list[r//10%20], K, inner_stepsize)
        iterates.append(iterates[-1] - outer_stepsize * direction)
        if (r+1) % loss_freq == 0:
            losses.append(objective_value(np.average(iterates,axis=0)))
            #print('Iteration: {:d}/{:d}   Loss: {:f}                 \r'.format(r+1,R,losses[-1]), end='')
...
def experiment(M,K,R,plotfile,savearray):
    ...
    np.savez(savearray, local_l = local_l, delay_l = delay_l, less_l = less_l, prox_l = prox_l, random_l = random_l, local_a = local_a, delay_a = delay_a, less_a = less_a, prox_a = prox_a, random_a = random_a)
