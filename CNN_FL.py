...
def experiment(M,K,R,plotfile,savearray):
    ...
    np.savez(savearray, local_l=local_results, delay_l=delay_results, less_l=less_results, prox_l=prox_results, random_l=random_results,
             local_a=local_results_acc, delay_a=delay_results_acc, less_a=less_results_acc, prox_a=prox_results_acc, random_a=random_results_acc)

    x0 = create_weight_matrices()
    l0_tmp, a0 = evaluate(x0, features, labels_array)
    l0 = l0_tmp - fstar
    ...
    for r in range(R // loss_freq):
        current_T += loss_freq * Tsingle_random[r % 10]
...
if learning_rate == 'grid_search':
    n_stepsizes = 4
    tt_stepsizes = [np.exp(exponent) for exponent in np.linspace(-6, -3, n_stepsizes)]
    lg_stepsizes = [np.exp(exponent) for exponent in np.linspace(-6, -3, n_stepsizes)]
    lc_stepsizes = [np.exp(exponent) for exponent in np.linspace(-6, -3, n_stepsizes)]
else:
...
experiment(M,K=5,R=200,plotfile= plot_prefix + '/smallM-smallK-' + distribution + dataset_name + '-CCN.png',savearray=plot_prefix + '/smallM-smallK-' + distribution + dataset_name+'-CCN')
